                            ...
                            match payload {
                                NetworkPayload::TransactionInv(tx_hash) => {
                                    ...
                                    if !is_known && !in_flight_txs.contains(&tx_hash) {
                                        ...
                                        // FIX: Two-stage buffer retry for Mempool. Guarantees delivery, prevents Yamux flooding.
                                        tokio::spawn(async move {
                                            let req = crate::network::SyncRequest::GetMempoolTx { tx_hash, requester: requester_str.clone() };
                                            let _ = swarm_cmd_tx_clone.send(SwarmCommand::SendSyncReq(peer_clone, req, Some(tx_hash))).await;
                                            ...
                                            if !is_solved {
                                                tracing::debug!("[WARN] Mempool: TX delayed. Stage-2 retrieval for {:?}", tx_hash);
                                                let req_retry = crate::network::SyncRequest::GetMempoolTx { tx_hash, requester: requester_str };
                                                let _ = swarm_cmd_tx_clone.send(SwarmCommand::SendSyncReq(peer_clone, req_retry, Some(tx_hash))).await;
                                            }
                                        });
                                    }
                                }
